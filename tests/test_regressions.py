...
class FrontendContractTests(unittest.TestCase):
    ...
    def test_storyboard_cards_show_subject_before_set_details(self):
        ...
        html = (root / "client" / "client.html").read_text(encoding="utf-8")
        js = (root / "client" / "client.js").read_text(encoding="utf-8")
        self.assertIn('id="studio-count" title="0 sets" aria-label="0 sets"', html)
        self.assertIn('id="director-count" title="0 shots" aria-label="0 shots"', html)
        self.assertIn('id="output-count" title="0 proofs" aria-label="0 proofs"', html)
        self.assertIn('id="log-count" title="0 log entries" aria-label="0 log entries"', html)
        ...
        self.assertIn("function updateNavigationCount(id, count, singular, plural", js)
        self.assertIn("updateNavigationCount('output-count', state.outputs.length, 'proof')", js)
        self.assertIn("updateNavigationCount('log-count', logs.length, 'log entry', 'log entries')", js)
        self.assertIn("updateNavigationCount('studio-count', navigationSetCount, 'set')", js)
        self.assertIn("updateNavigationCount('director-count', board.shots.length, 'shot')", js)
    ...
    def test_logger_timeline_can_inspect_historical_shot_prompts(self):
        ...
        self.assertIn("function mergeSessionLog(jobs = [], sessionLog = [])", js)
        self.assertIn("function selectedLoggerJob()", js)
        self.assertIn("function loggerEvents()", js)
        self.assertIn("function renderLoggerEvents(preview = null)", js)
        self.assertIn('data-job-id="${escapeHtml(eventJob.id)}"', js)
        self.assertNotIn('id="logger-session"', html)
        self.assertNotIn("function renderLoggerSession()", js)
        ...
        self.assertIn("image.hidden = !showImage", js)
        self.assertIn("video.hidden = !showVideo", js)
        self.assertIn(".logger-rendered-frame img[hidden], .logger-rendered-frame video[hidden]", css)
    ...
    def test_render_queue_uses_noninteractive_placeholder_thumbnails_and_local_eta(self):
        ...
        self.assertIn("`Video ${item.shot ?? item.position ?? 1}`", js)
        self.assertIn(": `Shot ${displayShot}`)", js)
        ...
        self.assertIn("video-progress-estimating", js)
        self.assertIn("video-progress-sweep", css)
        self.assertIn("setTimeout(pollJob, 1000)", js)
        self.assertIn("left.status === 'running' ? -1 : 1", js)
        self.assertIn("pendingVideo = item.pending && item.generation_mode === 'video'", js)
    ...
    def test_output_gallery_groups_photoshoots_by_run_and_set_filename(self):
        ...
        self.assertIn("function closePhotoshoot()", js)
        self.assertIn("if (event.key !== 'Escape' || !state.galleryGroup) return", js)
        self.assertIn("rememberProofsPosition();", js)
        self.assertIn("restoreProofsPosition({ fallbackToGrid: true });", js)
